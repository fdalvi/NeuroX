...
def load_data_rel(source_path, labels_path, head_path, activations, max_sent_l, ignore_start_token=False, sentence_classification=False):
    ...
    with open(head_path) as fp:
        for line in fp:
            ...
            if ignore_start_token:
                line_tokens = line_tokens[1:]
            tokens['head'].append(line_tokens)

    assert len(tokens['source']) == len(tokens['target']) and len(tokens['source']) == len(tokens['head']), \
        "Number of lines do not match (source: %d, target: %d, head: %d)!"%(len(tokens['source']), len(tokens['target']), len(tokens['head']))
    ...
    for num_deleted, idx in enumerate(invalid_activation_idx):
        print("Deleting line %d: %d activations, %d source, %d target"%
            (idx - num_deleted, 
                activations[idx - num_deleted].shape[0], 
                len(tokens['source'][idx - num_deleted]),
                len(tokens['head'][idx - num_deleted]), 
                len(tokens['target'][idx - num_deleted])
            )
        )
        del(activations[idx - num_deleted])
        del(tokens['source'][idx - num_deleted])
        del(tokens['target'][idx - num_deleted])
        del(tokens['head'][idx - num_deleted])
...
def load_aux_data_rel(source_path, labels_path, source_aux_path, head_path, activations, max_sent_l, ignore_start_token=False):
    ...
    with open(head_path) as fp:
        for line_idx, line in enumerate(fp):
            line_tokens = line.strip().split()
            if len(line_tokens) > max_sent_l:
                print("Skipping line #%d because of length (head)"%(line_idx))
                skipped_lines.add(line_idx)
            if ignore_start_token:
                line_tokens = line_tokens[1:]
            tokens['head'].append(line_tokens)
    ...
    assert len(tokens['source_aux']) == len(tokens['source']) and len(tokens['source_aux']) == len(tokens['target']) and len(tokens['source_aux']) == len(tokens['head']), \
        "Number of lines do not match (source: %d, aux: %d, target: %d, head: %d)!"%(len(tokens['source']), len(tokens['source_aux']), len(tokens['target']), len(tokens['head']))
    ...
    for num_deleted, line_idx in enumerate(sorted(skipped_lines)):
        ...
        del(tokens['source'][line_idx])
        del(tokens['target'][line_idx])
        del(tokens['head'][line_idx])

    # Check if all data is well formed (whether we have activations + labels for each and every word)
    invalid_activation_idx = []
    for idx, activation in enumerate(activations):
        if activation.shape[0] == len(tokens['source_aux'][idx]) and len(tokens['source'][idx]) == len(tokens['target'][idx]) and len(tokens['source'][idx]) == len(tokens['head'][idx]):
            pass
        else:
            invalid_activation_idx.append(idx)
            print ("Skipping line: ", idx, "A: %d, aux: %d, src: %d, tgt: %s, head: %s"%(activation.shape[0], len(tokens['source_aux'][idx]), len(tokens['source'][idx]), len(tokens['target'][idx]), len(tokens['head'][idx])))
    
    for num_deleted, idx in enumerate(invalid_activation_idx):
        print("Deleting line %d: %d activations, %s aux, %d source, %d head, %d target"%
            (idx - num_deleted, 
                activations[idx - num_deleted].shape[0], 
                len(tokens['source_aux'][idx - num_deleted]),
                len(tokens['source'][idx - num_deleted]),
    ...
    for idx, activation in enumerate(activations):
        assert(activation.shape[0] == len(tokens['source_aux'][idx]))
        assert(len(tokens['source'][idx]) == len(tokens['target'][idx]))
        assert(len(tokens['source'][idx]) == len(tokens['head'][idx]))
