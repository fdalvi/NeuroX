...
def load_data_and_train(train_source, train_aux_source, train_labels, train_head,train_activations,
                        test_source, test_aux_source, test_labels, test_head, test_activations,
                        exp_type, task_specific_tag, max_sent_l, n_epochs, batch_size,
                        is_brnn, filter_layers, ignore_start_token, sem_dep_rel, num_neurons_per_layer=500):
    ...
    if exp_type == 'word' or exp_type == 'charcnn' or exp_type == 'sent_last':
        train_tokens = data_loader.load_data_rel(train_source, train_labels, train_head, train_activations, max_sent_l, ignore_start_token=ignore_start_token, sentence_classification=(exp_type == 'sent_last'))
        test_tokens = data_loader.load_data_rel(test_source, test_labels, test_head, test_activations, max_sent_l, ignore_start_token=ignore_start_token, sentence_classification=(exp_type == 'sent_last'))
    else:
        # TODO: Implement sentence classification for subword systems
        train_tokens = data_loader.load_aux_data_rel(train_source, train_labels, train_aux_source, train_head, train_activations, max_sent_l, ignore_start_token=ignore_start_token)
        test_tokens = data_loader.load_aux_data_rel(test_source, test_labels, test_aux_source, test_head, test_activations, max_sent_l, ignore_start_token=ignore_start_token)

    NUM_TOKENS = sum([len(t) for t in train_tokens['target']])
    print('Number of total train tokens: %d'%(NUM_TOKENS))
    NUM_HEAD = sum([len(t) for t in train_tokens['head']])
    print('Number of total train heads: %d'%(NUM_HEAD))
    ...
    if sem_dep_rel:
       ...
       test_tokens, test_activations = repr.removeUnderScore(test_tokens, test_activations)
...
def main():
    ...
    parser.add_argument('--train-head', dest='train_head', required=True,
                    help='Location of train source head')
    ...
    parser.add_argument('--test-head', dest='test_head', required=True,
                    help='Location of test source head')
    ...
    parser.add_argument('--sem-dep-rel', dest='sem_dep_rel', default=False,
                    action='store_true', help='Semantic dependency relation')
    ...
    result = load_data_and_train(args.train_source, args.train_aux_source, args.train_labels, args.train_head, args.train_activations,
                        args.test_source, args.test_aux_source, args.test_labels, args.test_head, args.test_activations,
                        args.exp_type, args.task_specific_tag, args.max_sent_l, NUM_EPOCHS, BATCH_SIZE,
                        args.is_brnn, args.filter_layers, args.ignore_start_token, args.sem_dep_rel, args.num_neurons_per_layer, args.lambda_l1, args.lambda_l2)
