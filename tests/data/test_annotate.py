...
class TestCreateBinaryData(unittest.TestCase):
    def setUp(self):
        ...
        self.activations = [
            torch.rand((len(sentence), self.num_layers*768)) for sentence in self.tokens["source"]
        ]
    ...
    def test_basic_binary_annotation(self):
        ...
        words, labels, activations = annotate._create_binary_data(self.tokens, self.activations, {"this"})
    ...
    def test_basic_binary_annotation_word(self):
        ...
        test_word = "Hello"
        words, labels, activations = annotate._create_binary_data(self.tokens, self.activations, {test_word})
    ...
    def test_number_of_positive_negative_examples(self):
        ...
        test_word = "test"
        words, labels, activations = annotate._create_binary_data(self.tokens, self.activations, {test_word}, balance_data = True)
    ...
    def test_if_positive_class_is_zero(self):
        ...
        test_word = "abc"
        self.assertRaises(ValueError, annotate._create_binary_data, self.tokens, self.activations, {test_word})
    ...
    def test_if_negative_class_is_zero(self):
        ...
        self.assertRaises(ValueError, annotate._create_binary_data, self.tokens, self.activations, test_regex)

    def test_regex_based_annotation(self):
        "Basic regex based binary filter test"
        test_regex = re.compile(r'^\d$')
        words, labels, activations = annotate._create_binary_data(self.tokens, self.activations, test_regex)

        self.assertIn("1", words)
        self.assertIn("2", words)
        self.assertIn("3", words)

        self.assertTrue(torch.equal(activations[words.index("1")], self.activations[0][6,:]))
        self.assertTrue(torch.equal(activations[words.index("2")], self.activations[1][7,:]))
        self.assertTrue(torch.equal(activations[words.index("3")], self.activations[2][4,:]))

    def test_binary_filter_invalid(self):
        "Test if binary filter is wrongly specified"
        self.assertRaises(NotImplementedError, annotate._create_binary_data, self.tokens, self.activations, "abc")
    ...
    def test_warning_negative_class_is_smaller(self, mock_stdout):
        ...
        self.assertIn("WARNING: the negative class examples are less than the positive class examples", mock_stdout.getvalue())

    def test_function_based_annotation(self):
        "Basic function based binary filter test"
        test_fn = lambda x: "t" in x
        words, labels, activations = annotate._create_binary_data(self.tokens, self.activations, test_fn)
...
class TestAnnotateData(unittest.TestCase):
    def setUp(self):
        ...
        self.activations = [
            torch.rand((self.num_layers, len(sentence), 768)) for sentence in self.tokens["source"]
        ]
        self.tmpdir = TemporaryDirectory()
        data_utils.save_files(self.test_sentences, self.test_sentences, self.activations, f"{self.tmpdir.name}/gold", )


    @patch('neurox.data.annotate._create_binary_data')
    def test_binary_data_wrapper(self, mock_create_binary_data):
        mock_create_binary_data.return_value = self.test_sentences, self.test_sentences, self.activations

        annotate.annotate_data(f"{self.tmpdir.name}/gold.word", f"{self.tmpdir.name}/gold.hdf5", {"test"}, f"{self.tmpdir.name}/test")
        ...
        test_activations, test_num_layers = data_loader.load_activations(f"{self.tmpdir.name}/test.hdf5")
        ...
        for act_idx, act in enumerate(test_activations):
            self.assertTrue(torch.allclose(gold_activations[act_idx], torch.FloatTensor(act)))
