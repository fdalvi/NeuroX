...
class TestCreateSeqLabelingControlTaskDatasets(unittest.TestCase):
    ...
    def test_same_words_have_same_ct_label_in_same_dataset(self):
        ...
        self.assertEqual(ct_tokens["source"][0][3:], ct_tokens["source"][1][4:], msg="the moon") 
        self.assertEqual(ct_tokens["target"][0][3:], ct_tokens["target"][1][4:], msg="the moon") 
        self.assertEqual(ct_tokens["source"][1][0:4], ct_tokens["source"][2][1:5], msg="I am walking on")
        self.assertEqual(ct_tokens["target"][1][0:4], ct_tokens["target"][2][1:5], msg="I am walking on")
    ...
    def test_same_words_have_same_ct_label_in_diff_datasets(self):
        ...
        self.assertEqual(ct_tokens_train["source"][1][0:4], ct_tokens_dev["source"][0][0:4], msg="I am walking on")
        self.assertEqual(ct_tokens_train["target"][1][0:4], ct_tokens_dev["target"][0][0:4], msg="I am walking on")
    ...
    def test_num_ct_labels_exactly_matches_num_training_labels_two_labels(self):
        ...
        self.assertEqual(len(set(labels_flat)), 2, msg="number of ct labels == number of labels in the task training data")
    ...
    def test_num_ct_labels_roughly_matches_num_training_labels_three_labels(self):
        ...
        labels_flat = [l for sublist in ct_tokens["target"] for l in sublist]
        self.assertLessEqual(len(set(labels_flat)), 3, msg="number of ct labels <= number of labels in the task training data")
    ...
    def test_num_ct_labels_roughly_matches_num_training_labels_four_labels(self):
        ...
        labels_flat = [l for sublist in ct_tokens["target"] for l in sublist]
        self.assertLessEqual(len(set(labels_flat)), 4, msg="number of ct labels <= number of labels in the task training data")
    ...
    def test_uniform_distribution_matches(self):
        ...
        self.assertLess(max(label_freqs_dict.values()), 600, msg="Regardless of the label distribution in the training data, the control task labels are in a more or less uniform distribution")
    ...
    def test_same_distribution_matches(self):
        ...
        labels_flat = [l for sublist in ct_tokens["target"] for l in sublist]
        label_freqs_dict = Counter(labels_flat)
        self.assertLess(min(label_freqs_dict.values()), 200, msg="label distribution in the ct data roughly matches the label distribution in the training data")
