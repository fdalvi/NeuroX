...
def test_binary_guassian_ranking(binary_test_cases, snapshot):
    X, y = random.choice(
        binary_test_cases
    )  # test only on one case for guassian as it takes a lot of time to train a guassian probe
...
def test_binary_logistic_regression_probe_lca(binary_test_cases, snapshot):
    for idx, case in enumerate(binary_test_cases):
        X, y = case
        l_probe = linear_probe.train_logistic_regression_probe(
            X, y, lambda_l2=0.1, lambda_l1=0.1
        )
        overall_ranking, per_class_ranking = linear_probe.get_top_neurons(
            l_probe, 1, {"class0": 0, "class1": 1}
        )
        assert list(overall_ranking) == snapshot(
            name=f"binary_lca_overall_ranking_{idx}"
        )
        assert list(per_class_ranking["class0"]) == snapshot(
            name=f"binary_lca_class0_ranking_{idx}"
        )
        assert list(per_class_ranking["class1"]) == snapshot(
            name=f"binary_lca_class1_ranking_{idx}"
        )
...
def test_binary_logistic_regression_probe_no_reg(binary_test_cases, snapshot):
    for idx, case in enumerate(binary_test_cases):
        ...
        assert list(overall_ranking) == snapshot(
            name=f"binary_no_reg_overall_ranking_{idx}"
        )
        assert list(per_class_ranking["class0"]) == snapshot(
            name=f"binary_no_reg_class0_ranking_{idx}"
        )
        assert list(per_class_ranking["class1"]) == snapshot(
            name=f"binary_no_reg_class1_ranking_{idx}"
        )
...
def test_binary_logistic_regression_probe_lasso(binary_test_cases, snapshot):
    for idx, case in enumerate(binary_test_cases):
        ...
        assert list(overall_ranking) == snapshot(
            name=f"binary_lasso_overall_ranking_{idx}"
        )
        assert list(per_class_ranking["class0"]) == snapshot(
            name=f"binary_lasso_class0_ranking_{idx}"
        )
        assert list(per_class_ranking["class1"]) == snapshot(
            name=f"binary_lasso_class1_ranking_{idx}"
        )
...
def test_binary_logistic_regression_probe_ridge(binary_test_cases, snapshot):
    for idx, case in enumerate(binary_test_cases):
        ...
        assert list(overall_ranking) == snapshot(
            name=f"binary_ridge_overall_ranking_{idx}"
        )
        assert list(per_class_ranking["class0"]) == snapshot(
            name=f"binary_ridge_class0_ranking_{idx}"
        )
        assert list(per_class_ranking["class1"]) == snapshot(
            name=f"binary_ridge_class1_ranking_{idx}"
        )
...
def test_multiclass_probeless_ranking(multiclass_test_cases, snapshot):
    for idx, case in enumerate(multiclass_test_cases):
        ...
        ) = probeless.get_neuron_ordering_for_all_tags(X, y, idx2label=idx2label)
        assert overall_ranking == snapshot(
            name=f"probless_multiclass_ranking_case_{idx}"
        )
        for v in list(idx2label.values()):
            assert top_neurons_per_class[v] == snapshot(
                name=f"probeless_multiclass_ranking_case_{idx}_{v}"
            )
...
def test_multiclass_logistic_regression_probe_lca(multiclass_test_cases, snapshot):
    for idx, case in enumerate(multiclass_test_cases):
        ...
        overall_ranking, top_neurons_per_class = linear_probe.get_top_neurons(
            probe, 1, class_to_idx
        )
        assert list(overall_ranking) == snapshot(
            name=f"lca_probe_multiclass_overall_ranking_{idx}"
        )
        for k in list(class_to_idx.keys()):
            assert list(top_neurons_per_class[k]) == snapshot(
                name=f"lca_probe_multiclass_ranking_{idx}_{k}"
            )
...
def test_multiclass_logistic_regression_probe_no_reg(multiclass_test_cases, snapshot):
    for idx, case in enumerate(multiclass_test_cases):
        ...
        assert list(overall_ranking) == snapshot(
            name=f"no_reg_probe_multiclass_overall_ranking_{idx}"
        )
        for k in list(class_to_idx.keys()):
            assert list(top_neurons_per_class[k]) == snapshot(
                name=f"no_reg_probe_multiclass_ranking_{idx}_{k}"
            )
...
def test_multiclass_logistic_regression_probe_lasso(multiclass_test_cases, snapshot):
    for idx, case in enumerate(multiclass_test_cases):
        ...
        assert list(overall_ranking) == snapshot(
            name=f"lasso_probe_multiclass_overall_ranking_{idx}"
        )
        for k in list(class_to_idx.keys()):
            assert list(top_neurons_per_class[k]) == snapshot(
                name=f"lasso_probe_multiclass_ranking_{idx}_{k}"
            )
...
def test_multiclass_logistic_regression_probe_ridge(multiclass_test_cases, snapshot):
    for idx, case in enumerate(multiclass_test_cases):
        ...
        assert list(overall_ranking) == snapshot(
            name=f"ridge_probe_multiclass_overall_ranking_{idx}"
        )
        for k in list(class_to_idx.keys()):
            assert list(top_neurons_per_class[k]) == snapshot(
                name=f"ridge_probe_multiclass_ranking_{idx}_{k}"
            )
